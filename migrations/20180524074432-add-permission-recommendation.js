    ...
    async.waterfall([
        function(callback) {
            db.insert('permission', [
                {
                    "name": "vehicle-recommendation.sync-deleted",
                    "description": "Sync deleted recommendations from M1 DB",
            ...
            db._run('update', 'role', {
                    ...
                    update: {
                        $push: {
                            permissions: {
                                $each: [
                                    permissions['vehicle-recommendation.sync-deleted']._id
                                ]
                            }
                        }
                    },
...
exports.down = function(db, cb) {
    let pemissions = ['vehicle-recommendation.sync-deleted'];
